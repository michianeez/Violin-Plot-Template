...
#to visualise the aesthetic and informative violin plot, use the following code, but replace the relevant variable/column names/
...
#bam, then you're done, you've got one sexy graph. 
```

## Take care to keep ALL the column names the same when making new df's, otherwise the geom_point() and geom_errorbar() layers will not be added to the parent ggplot! This is because the values on the new df's need to be plotted on the axis of the parent df and hence need to be recognised by the parent df and ggplot. 
...
## Note: the real life example used for this graph type is not from "Lab 5 Clean Code"  
...
#voila, you're done, you've got sexy graph version 2.0 
```
